...
class Vineyard():
    ...
    def plot_vines(self, start_k = 0, end_k = None, include_represented_vines = False):
        ...
        return self._expand_represented_vines(seed_vines, include_represented_vines)

    def _expand_represented_vines(self, seed_vines, include_represented_vines = False):
    ...
    def plot_selected_vines(self, vine_ranks, label_by_region = False, show_legend = True, start_timestep = 0, end_timestep = None, exclude = [], region_colors = None, include_represented_vines = False, xlabel = "Birth", ylabel = "Death", zlabel = "Time"):
        ...
        seed_vines = []
        for rank in vine_ranks:
            rank = int(rank)
            if rank < 0 or rank >= len(ranked_vines):
                raise IndexError(f"vine rank {rank} is outside the valid range 0 to {len(ranked_vines) - 1}")
            seed_vines.append(ranked_vines[rank])
        plot_vines = self._expand_represented_vines(seed_vines, include_represented_vines)
    ...
    def _plot_vine_list(self, seed_vines, plot_vines, label_by_region = False, show_legend = True, start_timestep = 0, end_timestep = None, exclude = [], region_colors = None, xlabel = "Birth", ylabel = "Death", zlabel = "Time"):
        ...
        if label_by_region: 
            ...
            region_colors = {}
        max_birth = 0
        seed_vine_ids = {id(vine) for vine in seed_vines}
        if not plot_vines:
            plt.show()
            return region_colors if label_by_region else None

        def plot_vine_segment(birth_pair, death_pair, time_pair, color = None, alpha = 1, linewidth = 1.5):
            if color is None:
                return ax.plot(birth_pair, death_pair, time_pair, alpha = alpha, linewidth = linewidth)
            return ax.plot(birth_pair, death_pair, time_pair, color = color, alpha = alpha, linewidth = linewidth)
                ...
                if births[i] != deaths[i] or births[i+1] != deaths[i+1]:
                    if label_by_region:
                        ...
                        if not region in exclude:
                            birth_pair = [births[i], births[i+1]]
                            death_pair = [deaths[i], deaths[i+1]]
                            time_pair = [vertex_times[i], vertex_times[i+1]]
                            if region in region_colors:
                                plot_vine_segment(birth_pair, death_pair, time_pair, color = region_colors[region], alpha = vine_alpha, linewidth = vine_linewidth)
                            elif region in prev_region_colors:
                                p = plot_vine_segment(birth_pair, death_pair, time_pair, color = prev_region_colors[region], alpha = vine_alpha, linewidth = vine_linewidth)
                                region_colors.update({region : p[-1].get_color()})
                            else:
                                p = plot_vine_segment(birth_pair, death_pair, time_pair, alpha = vine_alpha, linewidth = vine_linewidth)
                                region_colors.update({region : p[-1].get_color()})
                    else:
                        birth_pair = [births[i], births[i+1]]
                        death_pair = [deaths[i], deaths[i+1]]
                        ...
                        if vinecolor is None:
                            ...
                            p = plot_vine_segment(birth_pair, death_pair, time_pair, color = vinecolor, alpha = vine_alpha, linewidth = vine_linewidth)
